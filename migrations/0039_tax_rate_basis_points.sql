...
UPDATE tax_rates SET rate_basis_points = CAST(rate * 10000 AS INTEGER) WHERE rate_basis_points IS NULL;
...
UPDATE order_items SET tax_rate_basis_points = CAST(tax_rate * 10000 AS INTEGER) WHERE tax_rate IS NOT NULL AND tax_rate_basis_points IS NULL;
