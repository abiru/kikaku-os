...
export default function SearchModal() {
	...
	return (
		...
		>
			{/* Backdrop */}
			<div
				className="fixed inset-0 bg-neutral-500/75 backdrop-blur-sm"
			...
			<div className="fixed inset-x-0 top-0 p-4 sm:p-6 md:p-20">
				<div className="mx-auto max-w-2xl transform overflow-hidden rounded-xl bg-white shadow-2xl">
					...
					<div className="relative">
						...
						{loading && (
							<div className="absolute right-4 top-3.5">
								<svg className="h-5 w-5 animate-spin text-neutral-400" viewBox="0 0 24 24">
					...
					{query.length >= 2 && (
						<div className="border-t border-neutral-100" data-testid="search-results">
							{error && !loading ? (
								...
								<div ref={resultsRef} id={listboxId} role="listbox" aria-label={t('search.results')} className="max-h-80 overflow-y-auto py-2">
									{results.map((product, index) => {
										...
										return (
											...
											>
												<div className="h-12 w-12 flex-shrink-0 rounded-lg bg-neutral-100 flex items-center justify-center overflow-hidden">
													{product.image ? (
														<img
															src={product.image}
															alt={product.title}
					...
					<div className="flex items-center justify-between border-t border-neutral-100 px-4 py-2.5 text-xs text-neutral-500">
						...
						<div className="flex items-center gap-2">
							<kbd className="rounded bg-neutral-100 px-1.5 py-0.5 font-mono text-[10px]">Enter</kbd>
							<span>{t('search.toSelect')}</span>
						</div>
						<div className="flex items-center gap-2">
							<kbd className="rounded bg-neutral-100 px-1.5 py-0.5 font-mono text-[10px]">Esc</kbd>
