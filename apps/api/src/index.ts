...
const runDailyCloseArtifacts = async (env: Env['Bindings'], date: string) => {
  ...
  await putText(env.R2, htmlKey, html, 'text/html');
  ...
  if (report.anomalies.level === 'warning' || report.anomalies.level === 'critical') {
    await env.DB.prepare(
      `INSERT INTO inbox_items (title, body, severity, status, created_at, updated_at)
       VALUES (?, ?, ?, 'open', datetime('now'), datetime('now'))`
    ).bind(
      'Daily close anomaly',
      `date=${date} diff=${report.anomalies.diff}`,
      report.anomalies.level
    ).run();
  }
};
