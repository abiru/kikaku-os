...
aiWorkflows.get('/workflows/usage', async (c) => {
  try {
    ...
    type UsageRow = { service: string; operation: string; requests: number; tokens: number; cost: number };
    const stats = (result.results || []) as UsageRow[];
    const totalCost = stats.reduce((sum, item) => sum + (item.cost || 0), 0);
    const totalTokens = stats.reduce((sum, item) => sum + (item.tokens || 0), 0);
    const totalRequests = stats.reduce((sum, item) => sum + (item.requests || 0), 0);
